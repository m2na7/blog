---
title: '토스트 번들링과 테스트, CI/CD - React Compact Toast(2)'
...
> **React Compact Toast**는 React 애플리케이션에서 간편하고 경량화된 토스트 알림을 제공하기 위해 제작한 오픈소스 라이브러리예요. <br/><br/>
...
이번 포스트에서는 React Compact Toast를 오픈소스 라이브러리로 만들어가는 과정에 대해 작성했어요.  
[**토스트 컴포넌트 구현기는 1편을 참고해주세요.**](http://m2na.dev/posts/react-compact-toast-part1)
...
안정적인 배포를 위해 기본적인 테스트와 검증도 철저히 진행했어요.
...
> **2025년 8월 이후 tsup은 더이상 적극적으로 유지보수하지 않는다고 선언했어요.**
>
> 현재 react-compact-toast는 tsup대신 **tsdown**을 사용해 번들 크기를 최적화했어요.  
> 자세한 내용은 다음 포스트에서 추가적으로 확인 가능해요!

## 4. 번들 크기 최적화

이제 해결해야 하는 중요한 문제는 **번들 크기 최적화**예요.  
기존에 가장 많은 유저가 사용하는 토스트 라이브러리의 번들 크기(Bundlephobia, gzipped 기준)는 다음과 같았어요.

- **sonner**: 9.3KB
- **react-toastify**: 6.9KB
- **react-hot-toast**: 5.2KB

저는 이 라이브러리들보다 최소 2배는 가벼운 라이브러리를 목표로 삼았어요.
...
위의 예시 코드에서 Tree-shaking이 제대로 동작한다면 `eventManager` 관련 코드는 최종 번들에서 빠져야 해요.
...
tsup과 tsdown은 기본적으로 사용되지 않는 코드(ex. `eventManager`)를 최종 번들에서 제거해요. (Tree-shaking 기본 적용)
...
> **9월 21일 기준 react-compact-toast는 약 1.6KB까지 번들 크기를 최적화했어요.**  
> 자세한 내용은 다음 포스트에서 추가적으로 확인 가능해요!
...
이제 마지막으로 CI/CD 환경을 구축해야 해요.  
매번 수동으로 `npm publish`를 실행하는 것은 번거롭고 실수가 발생할 가능성이 있기 때문에,
**GitHub Actions를 활용해 배포를 관리**하도록 설정했어요.

관련 yaml코드는 [여기서 확인](https://github.com/m2na7/react-compact-toast/tree/main/.github/workflows)할 수 있어요.
...
정식 버전 배포는 수동으로 워크플로우를 트리거하며,
배포 시 입력한 버전을 기준으로 자동으로 GitHub 릴리즈 태그가 생성되고 npm에도 배포되도록 설정했어요.

즉, 배포 과정은 수동 트리거지만, 직접 버전을 선택해 배포할 수 있기 때문에 버전 관리가 유연하고,
릴리즈 태그와 npm 배포까지 자동으로 처리되어 안정적으로 배포할 수 있어요.
...
그만큼 보람도 컸어요.  
왜냐하면 실제로 다른 개발자가 라이브러리를 사용하며 피드백을 줬던 매우 감사한 일이 생겼거든요...ㅎㅎ

한 예로, Toast 위치를 커스텀하지 못하는 문제가 제보되었고 offset 옵션을 활용해 빠르게 수정했던 경험이 기억에 남아요.  
